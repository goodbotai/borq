...
function getRapidProContact(contactUUID) {
  ...
  return fetch(url, {
    ...
    .catch((reason) =>
           genericCatchRejectedPromise(`GET Rapidpro contact: ${reason}`));
}
...
function getFacebookProfile(fbMessengerId) {
  ...
  return fetch(url)
    .then(extractJsonFromResponse)
    .catch((reason) =>
           genericCatchRejectedPromise(`GET Facebook profile: ${reason}`));
}
...
function postContactToRapidpro(payload, messengerId) {
    ...
    .catch((reason) =>
           genericCatchRejectedPromise(`POST Contact to Rapidpro: ${reason}`));
}
...
function genAndPostRapidproContact(groups, language, messengerId, extraFields) {
    ...
    .catch((reason) =>
           genericCatchRejectedPromise('GET Facebook profile in ' +
                                       `genAndPostRapidproContact: ${reason}`));
}
...
function postSubmissionToOna(payload) {
  ...
  fetch(onaSubmissionEndpointURL,
    ...
    .catch((reason) =>
           genericCatchRejectedPromise(`POST submission to Ona: ${reason}`));
}
...
function genAndPostSubmissionToOna(convo) {
  ...
  getFacebookProfile(Config.facebookApiVersion,
    ...
    .catch((reason) =>
           genericCatchRejectedPromise('GET Facebook profile in ' +
                                      `genAndPostSubmissionToOna: ${reason}`));
}
