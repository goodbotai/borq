...
*/
function getRapidProContact({uuid, urn}, baseURL = rapidproURL) {
...
* @return {promise} a promise for a rapidpro contact
*/
function updateRapidProContact({uuid, urn}, payload, baseURL = rapidproURL ) {
  // should just call postContactToRapidpro
  const queryParam = uuid ? `uuid=${uuid}` : `urn=${urn}`;
  return http.request(
    `${baseURL}?${queryParam}`,
    {method: 'POST',
...
function getFacebookProfile(fbMessengerId, baseURL = facebookURL) {
  const url = `${baseURL}${config.facebookApiVersion}/${fbMessengerId}` +
...
*/
function postContactToRapidpro(payload, messengerId, baseURL = rapidproURL) {
  return http.request(`${baseURL}?urn=facebook:${messengerId}`,
                      {
                        method: 'POST',
                        body: JSON.stringify(payload),
                        headers: {
...
* @param {object} user - Object containing the name of the messenger user
* @param {string} url - change the URL for testing
* @return {promise} a promise holding the request response
*/
