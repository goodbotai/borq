...
*/
function updateContact({uuid, urn}, payload, baseURL = rapidproURL ) {
...
* @param {string} rapidproURL - rapidpro url endpoint to post to
* @return {promise} a promise holding the request response
*/
function postRapidPro(payload, rapidproURL) {
  return http.request(rapidproURL,
                      {
                        method: 'POST',
...
*/
function getContact({uuid, urn}, baseURL = rapidproURL) {
  const queryParam = uuid ? `uuid=${uuid}` : `urn=${urn}`;
  return http.request(
    `${baseURL}?${queryParam}`,
